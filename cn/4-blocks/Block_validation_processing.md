# 区块的验证
与交易一样，区块也由网络上的每个节点进行验证。验证的目标是每个区块都可以在任意节点上独立地进行验证，且不依赖于任何外部特征。有关交易验证的更多信息，请查看[第3章](../3-transaction)。

# 区块验证
##调用和验证脚本
网络上的每个全节点都从其对等节点那里接收完整的NEO区块链数据。对于接收到的每个区块，它都会独立地执行验证，以确保接收到的区块不是由恶意节点或故障节点发送的。这样，节点就不需要信任任何其他的节点，创建了一个安全且去信任的系统。对于收到的每个区块，节点会通过使用验证脚本来验证调用脚本的方式，对区块中的所有交易进行验证。可以将这个调用脚本视为能解锁UTXO的密钥，证明了该脚本有权限花费这个UTXO，同时将验证脚本作为工具传递给每个节点进行验证。有关交易验证的更多信息，请查看[第3章](../3-transaction)。

除了每个交易内的调用脚本和验证脚本之外，每个区块本身也包含有一个调用脚本和验证脚本。验证脚本有时也被称为*见证人*。

当我们查看[第2部分，区块结构](2-Structure_of_a_block.md)中所介绍的高度为3,649,960的区块时，我们可以在“script”字段中看到这两个脚本。为方便起见，把这部分提取出来了：
...
```
这看起来似乎是一些完全随机的神秘代码。 然而，这种疯狂其实是有理可循的。 让我们再深入挖掘下。
...
1.我们从第一个字节开始：`0x55`。 根据我们的OpCodes表，我们发现：
```
/// <summary>
/// 数字5被推送至堆栈
/// </ summary >
...
2.接下来：`0x21`。 获取接下来的33个字节并将它们推送至堆栈。
```
/// <summary>
/// 将33个字节推送至计算栈
/// </summary>
...
5.快成功了！ 现在我们只剩下最后2个字节了。 下一个是`0x57`。 所以我们向堆栈加入数字7。
```
/// <summary>
/// 数字7被推送至堆栈
/// </summary>
...
6.要解析的最后一个字节是`0xae`。 让我们最后一次查看下这个字节码：
```
/// <summary>
...
## 解释
我们做到了！我们已经了解了调用和验证脚本中的内容，现在让我们来看看它们的含义。这些脚本的目的是允许任意节点都能验证区块实际上是由共识节点创建并达成共识的。为此，共识节点会使用自己的私钥对区块进行签名。当7个共识节点中有5个对同一区块进行了签名的话，就表示节点达成了共识且该区块是有效的。这5个签名是包含在调用脚本内的签名。

现在我们需要做的就是验证这5个签名的有效性，以便对这个区块进行验证。为此，区块内会包含有验证脚本。通过这种方式，区块就可以告知每个节点如何执行验证。因此验证区块的节点会执行验证脚本，如果脚本通过，则该区块会被标记为有效。

我们已经了解了验证脚本的含义。其中使用到了创建这个特定区块所涉及到的7个共识节点的公钥。这种方式是使用了类似于多方签名交易的概念，并将这个应用于区块中。最后一个操作码`CHECKMULTISIG`使得前面所有的一切都变得有意义。语法被解释为七分之五的多方签名，因此堆栈上的7个33字节长的字符串因此会被解释为这7个节点的公钥。

如果可以使用验证脚本中七分之五的公钥来验证调用脚本中的5个签名，则认为区块是有效的。