...
NEO交易是一个带有网络操作指令的签名数据包，例如，用户表明想要将资产转移到另一个地址。区块链分类账本中的每个NEO区块都包含有一个或多个交易，使得每个区块可以对交易进行批处理。
...
| `type`       | uint8   | 交易的类型             |
| `version`    | uint8   |兼容版本                 |
| `attributes` | array   |交易的其他属性  |
| `outputs`    | array   |附加资产的接收地址       |
| `inputs`     | array   |附加到交易的资产   |
| `scripts`    | array   |用于验证交易的脚本|

对于所有交易而言，都会计算出唯一的交易哈希`txid`（uint256）和`size`（uint8）参数。
...
版本属性允许对交易结构进行更新，使其具有向后兼容性。 目前版本`0`和`1`均支持`InvocationTransaction`交易，而其他类型的交易只支持在版本`0`中使用。
...
| 使用类型    | 名称| 说明| 长度|
|---------------|-------------------|---------------------------------------|-----------------------------------------|
| `0`           | `ContractHash`    | 合约的脚本哈希          | `32`                                    |
| `2`           | `ECDH02`          | 用于ECDH密钥交换的公钥    | `32`                                    |
| `3`           | `ECDH03`          | 用于ECDH密钥交换的公钥   | `32`                                    |
| `32`          | `Script`          | 交易额外的验证| `20`                                    |
| `48`          | `Vote`            | 投票payload                        |应该指定(最多255个字节)   |
| `129`         | `DescriptionUrl`  | 描述说明的URL                | 应该指定 (最多255个字节)   |
| `144`         | `Description`     | 说明|应该指定 (最多255个字节   |
| `161` - `175` | `Hash1-Hash15`    | 自定义的存储哈希值        | `32`                                    |
| `240` - `255` | `Remark-Remark15` | 自定义的一般性备注             |应该指定 (最多65535个字节) |
...
| 类型| 说明|
|-----------------------------|-------------------------------------------------------------------------------------------------|
| `IssueTransaction`          |要求交易由资产管理合约进行签名                   |
| `ClaimTransaction`          |需要在交易的`claims`属性中引用一个或多个未领取的输出   |
| `MinerTransaction`          |要求`inputs`为空，并且`outputs`中的金额与区块中的所有系统费用相匹配|                                                                 
...
| 字段 | 说明|
|----------------------|---------------------------------------------------------------------------------------------|
| `InvocationScript`   |给验证脚本推送有效签名的脚本                    |
| `VerificationScript` |用于推送与（多签名）合约相对应的公钥的验证脚本|

如果交易是`ContractTransaction`类型，其中资产附加在`inputs`属性中，那么将会对属于该输入的签名和公钥进行验证。 可以为每个交易添加多个见证人，也可以使用具有多签名的见证人。
