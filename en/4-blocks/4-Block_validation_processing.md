...
Just like transactions, blocks are validated by every node on the network. The goal for this validation is that every block can independently be validated on any node, and is not depending on any external characteristics, just like transactions. For more information on transaction validations, have a look at [Chapter 3](../3-transaction).

# Block Verification
## Invocation and Verification Script
Every full node on the network receives the full Neo blockchain from its peers. For every block it receives, it will independently validate this block to ensure that the block received was not sent by either a malicious or faulty node. This way the nodes don't trust any other node, but rather the trust and security is in the fact that this is a trustless system. For every block received, the node will validate all individual transactions in that block by means of using the Verification Script to validate the Invocation Script. This Invocation Script can be seen as the Key to unlock the UTXO, proving that it has the right to spend it, and passing the Verification Script as a tool for every node to validate this. For more information on transaction validation, have a look at [Chapter 3](../3-transaction).

Apart from the Invocation Script and Validation Script on every individual transaction, every block itself also contains an Invocation Script and Validation Script. The Validation Script it sometimes referred to as the *Witness* as well.
...
## Opcodes
The opcodes for Neo can be found [here](https://github.com/neo-project/neo-vm/blob/master/src/neo-vm/OpCode.cs). When validating a block, these opcodes tell the NeoVM how to proceed. Let's take the verification string to dig into. As this is a HEX representation of the byte code, we take it byte per byte. So let's start demystifying our verification code. The way it works is you interpret every code and push it onto the execution stack. This stack eventually gets executed by the NeoVM to validate the block.
```   
...
1. We start with the first byte: `0x55`. According to our table of OpCode, we find:
```
/// <summary>
...
4. After these 33 bytes, we again code at an OpCode `0x21` (...6662d4a59ad548df0e7d**21**025bdf3f...). This again means to take the next 33 bytes and push them on the stack. We have a total of 7 occurrences of opcode `0x21`, and thus 7 times 33 bytes that are pushed onto the stack.
...
## Interpretation
We've done it! Now we know what's inside both the Invocation and Verification Script, let's look at what it means. The goal of these scripts is allow any node to validate that the block was actually created and agreed upon by the consensus nodes. To do this, the consensus nodes sign the block with their private key. From the moment we have 5 of the 7 consensus nodes that signed the same block, we achieve consensus and the block is valid. These 5 signatures are the ones included in the Invocation Script.

Now all we need to be able to do, is validate that these 5 signatures are actually valid to validate the block. For that, the block containts the Verification Script. This way, the block tells every node how to validate it. The node that validates the block thus executes the Verification Script, and if this script passes, then the block is marked as valid.

We already found the meaning of the Verification Script. What happens is that the 7 public keys of the consensus nodes that were involved in creating this specific block are used. The wat this is done is with a concept similar to a multisig transaction, but then applied on a block. The last OpCode `CHECKMULTISIG` is the one that gives meaning to everything that came before. The syntax is thus as interpreted as a 5-out-of-7 multisig, and the 7 33-byte string on the stack are thus interpreted as the public keys of these 7 nodes.
