...
[NEO](https://neo.org/) is an open-source platform for decentralized applications. It is a blockchain with a [dBFT](https://docs.neo.org/en-us/basic/consensus/consensus.html) consensus protocol. It can run smart contracts and its native assets are NEO tokens and it uses GAS tokens to run contracts. It is similar to Ethereum, yet it differs in many aspects.
...
Although many [core libraries of NEO](https://github.com/neo-project/neo) are written in C# or Python, for this tutorial, we will us [Golang](https://golang.org/). The communication basics are the same for any language.
...
In order to participate in the NEO distributed system, first, we open a connection to NEO node. Since we want to issue ping/pong commands, which was released in NEO version 2.10.1 on 4. April 2019, we need to make sure that our node has this version. A list of NEO nodes with versions can be found here: [http://monitor.cityofzion.io/](http://monitor.cityofzion.io/). The following nodes announce this version:

* node1.plutolo.gy:10333
* seed.neoeconomy.io:10333
* seed10.ngd.network:10333

Please note, that those peers can go offline at any time, and those peers above may not be online anymore.

These nodes must be well reachable on the respective port. The default port in the mainnet is 10333. In case a node is behind a firewall, the node can use UPnP to configure your router to accept incoming connection to your node. NAT-PMP is currently not supported. In our tutorial we won't accept any incoming connection, that means we don't have any NAT issues.
...
```
Big-endian encoding would like as follows: [00][74][6E][41]. Many programming languages have utilities to do the conversion, such as Golang:
...
## Payload protocol
After implementing the encoding and decoding of the headers for the NEO protocol, we can focus on the payload. The NEO protocol supports the following commands. Not every command has a payload.
...
* getdata - request a specific object - will be replied in tx, block, merkleblock, or consensus message
...
* mempool - request verified transactions from the mempool - will be replied in inv
* ping - checks if peer alive
...
* Version (uint32) specifies the version of the protocol, currently version is 0.
...
## Protocol
With this information we can implement the NEO node ping. First we need to write the encoders and decoders for payload of version and ping.
...
```
We wrote the version and now we can expect the version from the remote host. In fact, some clients also send it right after connection establishment.
```
...
```
On receiving the version from the NEO node, we additionally check the checksum if they match. The checksum are the first 4 bytes of the double SHA256 hash of the payload.
```
...
```
After sending ping, we can expect a pong message.
```
	//////// receive pong
	read = make([]byte, 36)
